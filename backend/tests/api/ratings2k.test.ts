...
// rows are shaped like the route's select lists. SMALLINT arrives from pg as a
// real number, so nothing here is a numeric string.
const jokicSummary = {
  slug: 'nikola-jokic',
  name: 'Nikola Jokic',
  team: 'Denver Nuggets',
...
describe('GET /api/ratings2k/players', () => {
  ...
  // 1000, not the shared 500 default: the classic roster is ~767 cards and the
  // UI pages once per roster, so a 500 cap would hide its lowest-rated third.
  ...
  // regression: the ceiling used to be 500, which silently truncated the classic
  // roster (~767 cards) so its lowest-rated players could not be found at all.
  ...
  it('never lets an injected sort key reach the ordering clause', async () => {
    ...
    // assert — the sort is whitelisted, so the raw value is discarded
    expect(res.status).toBe(200);
...
describe('GET /api/ratings2k/players/:slug', () => {
  it('returns the card with its attributes, badges, and rating history', async () => {
    // arrange — player first, then attributes, badges, history
    queryMock
    ...
    // the oldest entry has nothing to diff against, so delta is genuinely null
  ...
  it('returns empty arrays for a card 2K has not rated yet', async () => {
    // arrange — four 2026 rookies have an overall and no attributes at all
...
describe('GET /api/ratings2k/by-player-name', () => {
  ...
  it('matches on the normalized name and binds it as a parameter', async () => {
    ...
    // act — accents and suffixes differ between 2K and stats.nba.com
    await request(app)
      .get('/api/ratings2k/by-player-name')
  ...
  it('prefers the current-roster card when several names collide', async () => {
    // arrange — "Gary Payton II" and "Gary Payton" both normalize to gary payton
    queryMock.mockResolvedValueOnce(pgResult([jokicSummary]));
  ...
  it('responds 200 with a null player when the name has no 2K card', async () => {
    ...
    // assert — a miss is expected, not an error the caller should branch on
    expect(res.status).toBe(200);

...
every model output travels with the cutoff of the model that produced it, and
validate_out_of_fold refuses any row whose cutoff is after its own game date.
nothing here does any splitting; evaluate.py owns the rolling-origin scheme.
"""
...
class AvailabilityModel:
    """the trained availability classifier.

    ``cutoff`` is the exclusive upper bound of the training window and travels
    with every probability this model emits.
    """
...
def assert_same_cutoff(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    individually out of fold is not enough: two different cutoffs mean the later
    half is in-fold with respect to rows the earlier half was honest about.
    """
...
def cross_fit_base_probabilities(
    ...
    the base model reads config.BASE_FEATURE_COLS only, with no teammate context of
    any kind, which is what makes it safe to build teammate context from it. blocks
    rather than a per-origin refit because the TRAINING rows' context features are
    what the final model is fitted on, so a per-origin scheme would move the leak
    from the metric into the fit. blocks too thin to fit fall back to the as-of safe
    stage-0 baseline, flagged in P_CONTEXT_SOURCE.
    """
    from .features import stage0_context_probability  # noqa: PLC0415
...
class EwmaProduction:
    """EWMA of whole-game totals over appearances, kept as a challenger baseline.

    do not reintroduce it into the composition: an average of past whole-game
    TOTALS already contains the minutes the player used to get, which makes it
    constant with respect to any minutes forecast. the value is precomputed in
    features.py, so this is a lookup plus a fallback, not a fit.
    """
...
class MinutesModel:
    ...
    fitted on APPEARANCE rows only: a non-appearance's zero is an availability fact
    the P(play) model already owns. ``kind='ewma'`` is the challenger baseline.
    """
...
class PerMinuteRate:
    ...
    the value is precomputed in features.py, so this is a lookup plus a fallback,
    not a fit. halflife and estimator default to config per stat; passing either
    explicitly overrides it, which is what the halflife sweep needs. the fallback is
    a league RATE, sum(stat)/sum(minutes), not a mean of per-player ratios, which
    would be dominated by players with three minutes of history.
    """
    ...
    def __init__(self, target: str, halflife: float | None = None,
        ...
        # an expanding mean has no halflife, recorded as NaN so a metadata reader
        # cannot mistake a leftover number for a memory claim
...
def snapshot_ewma_state(features: pd.DataFrame, cutoff: pd.Timestamp) -> pd.DataFrame:
    ...
    # both rate families are snapshotted, not only the promoted one per stat, so a
    # later re-read of the losing estimator does not need a full history replay
    cols += [
        col
...
def decomposed_estimate(scored: pd.DataFrame, conditional: np.ndarray) -> np.ndarray:
    ...
    ``scored`` must carry a validated out-of-fold P(play).
    """
...
def minutes_propagated_estimate(
    ...
    ``scored`` must carry both stamped quantities; all three guards run before
    anything is multiplied. the pair is returned together so the two numbers
    cannot be the same estimate computed twice in two places.
    """
    validate_out_of_fold(scored)
...
def coherence_clip(
    ...
    the box-score inequalities hold per game but not for the EXPECTATIONS, because
    each stat's per-minute rate is smoothed at its own halflife. constraint ORDER is
    load-bearing: FGM is pulled under FGA before FG3M is pulled under the
    already-corrected FGM. a missing stat on either side is skipped, not zeroed.
    """
...
def coherence_clip_frame(
    ...
    ``template`` is a format string with one ``{target}`` field, e.g. ``'E_{target}'``
    or ``'Q90_{target}'``; quantile levels are clipped level-wise against each other
    rather than against the point estimate.
    """
    columns = {
...
class DecomposedEstimator:
    ...
    three fitted pieces, one cutoff handed to both trained models.
    """

...
@pytest.fixture(scope="session")
def universe_status(schedule, team_logs, raw_logs, status) -> pd.DataFrame:
    return universe_from_status(schedule, team_logs, raw_logs, status)


@pytest.fixture(scope="session")
def universe_approx(schedule, team_logs, raw_logs) -> pd.DataFrame:
    return approximate_universe(schedule, team_logs, raw_logs)
